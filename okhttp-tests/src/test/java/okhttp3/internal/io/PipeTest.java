...
public final class PipeTest {
  ...
  @Test public void sinkTimeout() throws Exception {
    ...
    pipe.sink.timeout().timeout(500, TimeUnit.MILLISECONDS);
    pipe.sink.write(new Buffer().writeUtf8("abc"), 3L);
    double start = now();
    try {
  ...
  @Test public void sourceTimeout() throws Exception {
    ...
    pipe.source.timeout().timeout(500, TimeUnit.MILLISECONDS);
    double start = now();
    Buffer readBuffer = new Buffer();
    try {
   ...
   * sleeping 500 ms, then reading 3 bytes. That should make for an approximate timeline like this:
   *
  ...
  /**
   * The writer has 12 bytes to write. It alternates sleeping 500 ms, then writing 3 bytes. The
